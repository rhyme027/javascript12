...
var validateName = function() {
	if (name_info.value == "") {//验证是否为空
		...
		return;
	}else if (getLength(name_info.value) <4 || getLength(name_info.value)>16){
		//验证长度是否符合标准
		name_info.nextElementSibling.innerHTML = "长度只能为4~16个字符";
...
var validatePassword = function(){
	...
	if(password_confirm_info.value != ""){
		if(password_info.value === password_confirm_info.value){
			...
			password_confirm_info.style.border = "2px solid #888";
		}
	}else{
...
var validatePhone = function() {
	if(phone_info.value != ""){
		...
		phone_info.nextElementSibling.style.color = "#BBBBBB";
